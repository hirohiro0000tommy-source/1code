...
11. Open `公式ボット投稿`, click `おすすめだけ公開` first, and confirm the public feed shows `見本` and `公式` labels. Use `未投稿分を公開` only when the feed still needs more starting examples.
...
- Confirm real user posts are not confused with `見本` posts; official examples should remain clearly labeled. If real posts arrive, avoid adding more official examples.
