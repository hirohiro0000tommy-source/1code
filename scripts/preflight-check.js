...
if (server.includes("function officialBotDrafts") && server.includes("/api/admin/bot/drafts") && server.includes("/api/admin/bot/publish") && server.includes("launchTag") && server.includes("recruit-apex-short-no-vc") && server.includes("recruit-overwatch-role-queue") && server.includes("recruit-splatoon-salmon-run") && server.includes("recruit-pokemon-champions-practice") && server.includes("thread-tonight-game-checkin") && server.includes("drafts.slice(0, 16)") && app.includes("renderOfficialBot") && app.includes("bot-draft-summary") && app.includes("publish-bot-recommended") && app.includes("おすすめだけ公開") && app.includes("見本") && smoke.includes("official bot overwatch draft missing") && smoke.includes("official bot selected publish failed")) pass("official bot launch seed coverage");
