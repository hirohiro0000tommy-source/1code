...
if (server.includes("function officialBotDrafts") && server.includes("/api/admin/bot/drafts") && server.includes("/api/admin/bot/publish") && server.includes("launchTag") && server.includes("recruit-apex-short-no-vc") && server.includes("thread-tonight-game-checkin") && server.includes("drafts.slice(0, 12)") && app.includes("renderOfficialBot") && app.includes("bot-draft-summary") && app.includes("見本") && smoke.includes("official bot launch drafts missing") && smoke.includes("official bot selected publish failed")) pass("official bot launch seed coverage");
