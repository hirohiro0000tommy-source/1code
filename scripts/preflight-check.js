...
if (index.includes("application/ld+json") && index.includes("\"@type\": \"WebSite\"") && index.includes("1code") && server.includes("homeHtml") && smoke.includes("home structured data absolute url missing")) pass("home structured data");
...
if (packageJson.scripts?.["status:check"] && statusCheck.includes("/healthz") && statusCheck.includes("/status") && statusCheck.includes("/status.json") && statusCheck.includes("1code status") && statusCheck.includes("request") && statusCheck.includes("trace: status") && statusCheck.includes("release")) pass("light status check");
...
if (server.includes("function userDataExport") && server.includes("/api/me/export") && app.includes("download-my-data") && app.includes("1code-user-data-") && smoke.includes("user data export format failed")) pass("user data export");
