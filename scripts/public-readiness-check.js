...
if (server.includes("function publicOperationsReport") && server.includes("/api/admin/public-report") && server.includes("1code 公開運用メモ") && server.includes("launchManualChecks") && server.includes("launchWatchPlan") && server.includes("公開後手動確認") && server.includes("公開直後の監視")) pass("public operations report API");
...
if (server.includes("function deploymentHandoff") && server.includes("/api/admin/deployment-handoff") && server.includes("1code 外部サービス設定ハンドオフ") && server.includes("PUBLIC_SECURITY_CONTACT") && server.includes("Staff roles SQL") && server.includes("handoffSteps") && server.includes("envChecklist") && server.includes("安全な環境変数チェック") && server.includes("実行順") && smoke.includes("deployment handoff security contact item missing") && smoke.includes("deployment handoff staff role sql item missing") && smoke.includes("deployment handoff steps missing") && smoke.includes("deployment handoff env checklist missing")) pass("deployment handoff API");
...
if (launchDayRunbook.includes("Stop points") && launchDayRunbook.includes("First public share") && launchDayRunbook.includes("Emergency brake") && launchDayRunbook.includes("/status.json") && launchDayRunbook.includes("PUBLIC_WRITE_PAUSED=true")) pass("launch day runbook");
...
if (packageJson.scripts?.["launch:today"] && fileText("scripts/launch-today.js").includes("Launch-today local preparation passed") && fileText("scripts/launch-today.js").includes("deploy-verify.js") && fileText("scripts/launch-today.js").includes("generate-launch-packet.js") && fileText("scripts/launch-today.js").includes("公開設定ハンドオフ") && fileText("scripts/launch-today.js").includes("Share to a small group first")) pass("today launch npm script");
else fail("today launch npm script", "missing");

if (packageJson.scripts?.["launch:packet"] && fileText("scripts/generate-launch-packet.js").includes("1code launch packet") && fileText("scripts/generate-launch-packet.js").includes("launch-packet.md") && fileText("scripts/generate-launch-packet.js").includes("does not include secret values") && fileText("scripts/generate-launch-packet.js").includes("External setup order") && fileText("scripts/generate-launch-packet.js").includes("PUBLIC_WRITE_PAUSED=true")) pass("launch packet npm script");
