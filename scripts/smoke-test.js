...
async function run() {
  ...
  try {
    ...
    assert(Array.isArray(health.runtime?.recentSlowRequests), "health slow request history missing");
    ...
    assert(health.limits?.slowRequestThresholdMs >= 500, "health slow request threshold missing");
    assert(health.limits?.slowStorageThresholdMs >= 100, "health slow storage threshold missing");
    ...
    assert(homePage.body.includes("一人用お題箱") && homePage.body.includes("募集文メモ") && homePage.body.includes("gameCategoryToggle"), "entry tools missing");
    ...
    assert(appJs.body.includes("soloPrompts") && appJs.body.includes("buildRecruitmentMakerText") && appJs.body.includes("gameCategoryToggle"), "entry tool scripts missing");
    ...
    assert(stylesCss.body.includes(".solo-tools") && stylesCss.body.includes(".maker-grid") && stylesCss.body.includes(".game-category-body"), "entry tool styles missing");
